
// /*
//  * Public API Surface of base-services
//  */

export * from './lib/areas/http/http-base/enums/index';
export * from './lib/areas/http/http-base/services/index';
export * from './lib/areas/object-creation/decorators';
export * from './lib/areas/object-creation/services/object-factory.service';
export * from './lib/areas/ng-base-services.module';
export * from './lib/areas/http/in-memory-api/services';
export * from './lib/areas/http/in-memory-api/models';
export * from './lib/areas/local-storage/services';
export * from './lib/areas/local-storage/models';
export * from './lib/areas/app-settings/services';
export * from './lib/areas/auto-unsubscription/decorators';

